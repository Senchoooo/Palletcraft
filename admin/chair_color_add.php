...
<?php
	...
	if(isset($_POST['add'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM chair_color WHERE color=:color");
		...
		if($row['numrows'] > 0){
			...
			try{
				$stmt = $conn->prepare("INSERT INTO chair_color (color, price) VALUES (:color, :price)");
	...
	header('location: chair_color.php');
