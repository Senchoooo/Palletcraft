<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, customize_chair_order.id AS customid FROM customize_chair_order LEFT JOIN users ON users.id=customize_chair_order.user_id WHERE order_date2 BETWEEN '$from' AND '$to' AND customize_chair_order.status = 3 ORDER BY order_date2 DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
    ...
    //                     $pending = (!$row['status']) ? '<span class="pull-right"><a href="#activate" class="status" data-toggle="modal" data-id="'.$row['id'].'"><i class="fa fa-check-square-o"></i></a></span>' : '';
			...
			$contents .= '
			<tr>
				<td align="center">'.date('M d, Y', strtotime($row['order_date2'])).'</td>
				<td align="center">'.$row['firstname'].' '.$row['lastname'].'</td>
				<td align="center">'.$row['confirmation'].'</td>
				
                            <td align="center">'.$row['chair_shape'].'</td>
                            
                            <td align="center">'.$row['chair_color'].'</td>
                            <td align="center">'.$row['quantity'].'</td>
                            <td align="center"><b> &#8369;'.number_format($downpayment, 2).'</b></td>
	...
	if(isset($_POST['print'])){
	    ...
	    $content .= '
	      	...
	      	<h4 align="center">CHAIR CUSTOMIZATION OUT FOR DELIVERY REPORT</h4>
	        ...
	        <th width="11%" align="center"><b>Date</b></th>
					...
					<th width="15%" align="center"><b>Shape</b></th>
