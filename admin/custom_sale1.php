...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Out for Delivery Plant Stand Sales History
      </h1>
      <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
          <div class="box">
              ...
              <div class="pull-right">
                <form method="POST" class="form-inline" action="ps_outdel_print.php">
            ...
            <div class="box-body">
             <table class="table table-bordered" id="example1">
                    ...
                    <?php
                      ...
                      try{
                         $stmt = $conn->prepare("SELECT *, customization_order.id AS customid, stock_list.id AS stockid FROM customization_order LEFT JOIN users ON users.id=customization_order.user_id LEFT JOIN stock_list ON stock_list.id   WHERE customization_order.ordertype = 'delivery' ORDER BY order_date DESC");
                      $stmt->execute();
                        $total = 0;
...
$(function(){
  $(document).on('click', '.custom', function(e){
    e.preventDefault();
    $('#customizedeliver').modal('show');
    var id = $(this).data('id');
    $.ajax({
      type: 'POST',
      url: 'custom_deliver.php',
      data: {id:id},
      dataType: 'json',
      success:function(response){
        $('#plantstanddeliverdate').html(response.plantstanddeliverdate);
        $('#plantstanddeliverdate1').html(response.plantstanddeliverdate1);
        $('#customdeliverot').html(response.customdeliverot);
        $('#customdeliverda').html(response.customdeliverda);
        $('#customid1').val(response.customid);
        $('#customizedeliverid').html(response.customizedeliver);
        $('#customdeliverlist').prepend(response.customlist);
        $('#total').html(response.total);
      }
    });
  });

  $("#customizedeliver").on("hidden.bs.modal", function () {
