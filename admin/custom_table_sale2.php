        ...
        <div class="col-xs-12">
          <div class="box">
              ...
              <div class="pull-right">
                <form method="POST" class="form-inline" action="tablesale_print.php">
            ...
            <div class="box-body">
             <table class="table table-bordered" id="example1">
                    ...
                    <?php
                      ...
                      try{
                        $stmt = $conn->prepare("SELECT *, customize_table_order.id AS tableid FROM customize_table_order LEFT JOIN users ON users.id=customize_table_order.user_id WHERE customize_table_order.status = 4 ORDER BY order_date1 DESC");
                      $stmt->execute();
                        $total = 0;
...
$(function(){
  $(document).on('click', '.tablecustom', function(e){
    e.preventDefault();
    $('#tablereceive').modal('show');
    var id = $(this).data('id');
    $.ajax({
      type: 'POST',
      url: 'table_received.php',
      data: {id:id},
      dataType: 'json',
      success:function(response){
        $('#tablereceivedate').html(response.tablereceivedate);
        $('#tablereceivedate1').html(response.tablereceivedate1);
        $('#tableordertype2').html(response.tableordertype2);
          $('#tabledelivery2').html(response.tabledelivery2);
        $('#tablereceiveid').html(response.tablereceiveid);
        $('#tablereceivecustom').html(response.tablereceivecustom);
        $('#tablereceivelist').prepend(response.tablereceivelist);
        $('#total').html(response.total);
      }
    });
  });

  $("#tablereceive").on("hidden.bs.modal", function () {
