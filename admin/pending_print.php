<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, customization_order.id AS customid FROM customization_order LEFT JOIN users ON users.id=customization_order.user_id WHERE order_date BETWEEN '$from' AND '$to' AND customization_order.ordertype = 'pickup' ORDER BY order_date DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
	...
	if(isset($_POST['print'])){
	    ...
	    $content .= '
	      	...
	      	<h4 align="center">CUSTOMIZATION PICK UP ORDER REPORT</h4>
