<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, customize_order.id AS customid FROM customize_order LEFT JOIN users ON users.id=customize_order.user_id WHERE order_date BETWEEN '$from' AND '$to' AND customize_order.status = 4 ORDER BY order_date DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
    ...
    //                     $pending = (!$row['status']) ? '<span class="pull-right"><a href="#activate" class="status" data-toggle="modal" data-id="'.$row['id'].'"><i class="fa fa-check-square-o"></i></a></span>' : '';
			...
			<tr>
				...
				<td align="center">'.$row['confirmation'].'</td>
                            ...
                            <td align="center">'.$row['layer'].'</td>
                            <td align="center">'.$row['flatform_quantity'].'</td>
	...
	if(isset($_POST['print'])){
	    ...
	    $content .= '
	      	...
	      	<h4 align="center">PLANT STAND CUSTOMIZATION OUT FOR DELIVERY REPORT</h4>
	      	...
	      	<table border="1" cellspacing="0" cellpadding="5">  
	           <tr>  
					...
					<th width="7%" align="center"><b>Layer</b></th>
					<th width="12%" align="center"><b>Platform quantity</b></th>
