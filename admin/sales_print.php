<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, sales.id AS salesid FROM sales LEFT JOIN users ON users.id=sales.user_id WHERE sales_date BETWEEN '$from' AND '$to' AND sales.ordertype = 'pickup' ORDER BY sales_date DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
	...
	if(isset($_POST['print'])){
	    ...
	    $content .= '
	      	...
	      	<h4 align="center">PICK UP ORDER SALES REPORT</h4>
