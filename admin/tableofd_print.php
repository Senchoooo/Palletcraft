<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, customize_table_order.id AS customid FROM customize_table_order LEFT JOIN users ON users.id=customize_table_order.user_id WHERE order_date1 BETWEEN '$from' AND '$to' AND customize_table_order.status = 3 ORDER BY order_date1 DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
