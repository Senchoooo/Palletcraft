<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, customize_chair_order.id AS customid FROM customize_chair_order LEFT JOIN users ON users.id=customize_chair_order.user_id WHERE order_date2 BETWEEN '$from' AND '$to' AND customize_chair_order.status = 1 ORDER BY order_date2 DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
	...
	if(isset($_POST['print'])){
	    ...
	    $content .= '
	      	...
	      	<h4 align="center">CHAIR CUSTOMIZATION PENDING REPORT</h4>
