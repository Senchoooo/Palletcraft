...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Delivered Plant Stand Sales History
      </h1>
      <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
          <div class="box">
              ...
              <div class="pull-right">
                <form method="POST" class="form-inline" action="pssale_print.php">
            ...
            <div class="box-body">
             <table class="table table-bordered" id="example1">
                    ...
                    <?php
                      ...
                      try{
                        $stmt = $conn->prepare("SELECT *, customize_order.id AS customid FROM customize_order LEFT JOIN users ON users.id=customize_order.user_id WHERE customize_order.status = 4 ORDER BY order_date DESC");
                      $stmt->execute();
                        $total = 0;
...
$(function(){
  $(document).on('click', '.custom', function(e){
    ...
    $.ajax({
      ...
      dataType: 'json',
      success:function(response){
        $('#plantstanddatedone').html(response.plantstanddatedone);
        $('#plantstanddatedone1').html(response.plantstanddatedone1);
        $('#customreceivedone').html(response.customreceivedone);
        $('#customreceivedone2').html(response.customreceivedone2);
        $('#customizedone').html(response.customizedone);
        $('#customlistdone').prepend(response.customlistdone);
        $('#total').html(response.total);
      }
    });
  });

  $("#customdone").on("hidden.bs.modal", function () {
