        ...
        <div class="col-xs-12">
          <div class="box">
              ...
              <div class="pull-right">
                <form method="POST" class="form-inline" action="tablepending_print.php">
            ...
            <div class="box-body">
             <table class="table table-bordered" id="example1">
                    ...
                    <?php
                      ...
                      try{
                        $stmt = $conn->prepare("SELECT *, customize_table_order.id AS tableid FROM customize_table_order LEFT JOIN users ON users.id=customize_table_order.user_id WHERE customize_table_order.status = 1 ORDER BY order_date1 DESC");
                      $stmt->execute();
                        $total = 0;
...
$(function(){
  $(document).on('click', '.tablecustom', function(e){
    ...
    $.ajax({
      ...
      dataType: 'json',
      success:function(response){
        $('#tabledate').html(response.tabledate);
         $('#tableordertype').html(response.tableordertype);
          $('#tabledelivery').html(response.tabledelivery);
        $('#tableid').val(response.tableid);
        $('#tablecustomid').html(response.tablecustom);
        $('#tablelist').prepend(response.tablelist);
        $('#total').html(response.total);
      }
    });
  });

  $("#tablecustom").on("hidden.bs.modal", function () {
