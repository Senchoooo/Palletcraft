...
<div class="wrapper">
  ...
  <div class="content-wrapper">
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    ...
                    try{
                      $stmt = $conn->prepare("SELECT * FROM customize_flatform_quantity");
                      $stmt->execute();
                      foreach($stmt as $row){
    ...
    <?php include 'includes/platform_modal.php'; ?>
...
function getRow(id){
  $.ajax({
    type: 'POST',
    url: 'platform_row.php',
    data: {id:id},
    dataType: 'json',
    success: function(response){
      $('.platformid').val(response.id);
      $('#edit_quantity').val(response.flatform_quantity);
      $('#edit_price').val(response.price);
      $('.platforname').html(response.name);
    }
  });
}
</script>
