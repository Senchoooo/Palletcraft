            ...
            <div class="box-body">
             <table class="table table-bordered" id="example1">
                    ...
                    <?php
                      ...
                      try{
                        $stmt = $conn->prepare("SELECT *,  sales.id AS salesid FROM sales LEFT JOIN users ON users.id=sales.user_id WHERE sales.status = 4 ORDER BY sales_date DESC");
                      $stmt->execute();
                        ...
                        $done = (!$row['status']) ? '<span class="pull-right"><a href="#activate" class="status" data-toggle="modal" data-id="'.$row['id'].'"><i class="fa fa-check-square-o"></i></a></span>' : '';
                          ...
                          echo "
                            ...
                            <td>
                              ...
                              <td><button class='btn btn-sm btn-flat btn-info received' data-id='".$row['salesid']."'><i class='fa fa-search'></i> View</button>
...
$(function(){
  $(document).on('click', '.received', function(e){
    ...
    $.ajax({
      ...
      dataType: 'json',
      success:function(response){
        $('#date4').html(response.date4);
        $('#saleid').html(response.sale4);
        $('#ordertype2').html(response.ordertype2);
        $('#deliveryaddress2').html(response.deliveryaddress2);
        $('#dateDelivery2').html(response.dateDelivery2);
        $('#salesdelivered').html(response.salesdelivered);
        $('#saledetail').prepend(response.list);
        $('#saletotal').html(response.total);
      }
    });
  });

  $("#sale4").on("hidden.bs.modal", function () {
